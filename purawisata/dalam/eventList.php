...
if($_GET) {	
	...
	if($_GET["m"]=="ubahstatus") {
		$sql = "update ".tabel_event." set status = IF(status='1','0','1') where id='".$id."' ";
		...
		$sqlU="delete from ".tabel_event." where id='".$id."'";
		//echo $sqlU;
		mysql_query($sqlU,$tulis);
		
		if(file_exists('../images/event/'.$id.'.jpg'))  unlink('../images/event/'.$id.'.jpg');
		if(file_exists('../images/event/thumb/'.$id.'.jpg'))  unlink('../images/event/thumb/'.$id.'.jpg');
	}
}
...
$i = 0;
$sql = "select * from ".tabel_event." where 1 and kategori='0' ".$addSql." order by id desc ";
